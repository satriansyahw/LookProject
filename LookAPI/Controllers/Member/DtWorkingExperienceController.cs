...
namespace LookAPI.Controllers.Member
{
    ...
    public class DtWorkingExperienceController : Controller
    {
        ...
        {
            var hasil = await general.HapusActiveBoolAsync<DtWorkingExperience>(deleteByIdList.IdentityId, deleteByIdList.UserByName);
            return Ok(hasil);
        }
        [Route("SimpanListAsync")]
        [HttpPost("{listEntity}")]
        public async Task<IActionResult> SimpanListAsync([FromBody]List<DtWorkingExperience> listEntity)
        {
        ...
        [HttpPost("{listEntity}")]
        public async Task<IActionResult> UpdateListAsync([FromBody]List<DtWorkingExperience> listEntity)
        {
        ...
        {
            var hasil = await general.ListDataAsync<DtWorkingExperience, VW_DtWorkingExperience>(searchParam.SearchFieldList, searchParam.SortColumn, searchParam.IsAscending, searchParam.TopTake);
            return Ok(hasil);
        }
        ...
        {
            var hasil = await general.SearchByIdAsync<DtWorkingExperience>(id);
            return Ok(hasil);
        }
    }
}