...
namespace LookAPI.Controllers.Member
{
    ...
    public class DtEducationController : Controller
    {
        ...
        {
            var hasil = await general.HapusActiveBoolAsync<DtEducation>(deleteByIdList.IdentityId, deleteByIdList.UserByName);
            return Ok(hasil);
        }
        [Route("SimpanListAsync")]
        [HttpPost("{listEntity}")]
        public async Task<IActionResult> SimpanListAsync([FromBody]List<DtEducation> listEntity)
        {
        ...
        [HttpPost("{listEntity}")]
        public async Task<IActionResult> UpdateListAsync([FromBody]List<DtEducation> listEntity)
        {
        ...
        {
            var hasil = await general.ListDataAsync<DtEducation, VW_DtEducation>(searchParam.SearchFieldList, searchParam.SortColumn, searchParam.IsAscending, searchParam.TopTake);
            return Ok(hasil);
        }
        ...
        {
            var hasil = await general.SearchByIdAsync<DtEducation>(id);
            return Ok(hasil);
        }
    }
}