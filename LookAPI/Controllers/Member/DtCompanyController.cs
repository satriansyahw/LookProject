...
namespace LookAPI.Controllers.Member
{
    ...
    public class DtCompanyController : Controller
    {
        ...
        {
            var hasil = await general.HapusActiveBoolAsync<DtCompany>(deleteByIdList.IdentityId, deleteByIdList.UserByName);
            return Ok(hasil);
        }
        [Route("SimpanListAsync")]
        [HttpPost("{listEntity}")]
        public async Task<IActionResult> SimpanListAsync([FromBody]List<DtCompany> listEntity)
        {
        ...
        [HttpPost("{listEntity}")]
        public async Task<IActionResult> UpdateListAsync([FromBody]List<DtCompany> listEntity)
        {
        ...
        {
            var hasil = await general.ListDataAsync<DtCompany, VW_DtCompany>(searchParam.SearchFieldList, searchParam.SortColumn, searchParam.IsAscending, searchParam.TopTake);
            return Ok(hasil);
        }
        ...
        {
            var hasil = await general.SearchByIdAsync<DtCompany>(id);
            return Ok(hasil);
        }
    }
}