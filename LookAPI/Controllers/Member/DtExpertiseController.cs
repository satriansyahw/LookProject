...
namespace LookAPI.Controllers.Member
{
    ...
    public class DtExpertiseController : Controller
    {
        ...
        {
            var hasil = await general.HapusActiveBoolAsync<DtExpertise>(deleteByIdList.IdentityId, deleteByIdList.UserByName);
            return Ok(hasil);
        }
        [Route("SimpanListAsync")]
        [HttpPost("{listEntity}")]
        public async Task<IActionResult> SimpanListAsync([FromBody]List<DtExpertise> listEntity)
        {
        ...
        [HttpPost("{listEntity}")]
        public async Task<IActionResult> UpdateListAsync([FromBody]List<DtExpertise> listEntity)
        {
        ...
        {
            var hasil = await general.ListDataAsync<DtExpertise, VW_DtExpertise>(searchParam.SearchFieldList, searchParam.SortColumn, searchParam.IsAscending, searchParam.TopTake);
            return Ok(hasil);
        }
        ...
        {
            var hasil = await general.SearchByIdAsync<DtExpertise>(id);
            return Ok(hasil);
        }
    }
}