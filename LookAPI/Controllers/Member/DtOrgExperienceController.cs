...
namespace LookAPI.Controllers.Member
{
    ...
    public class DtOrgExperienceController : Controller
    {
        ...
        {
            var hasil = await general.HapusActiveBoolAsync<DtOrgExperience>(deleteByIdList.IdentityId, deleteByIdList.UserByName);
            return Ok(hasil);
        }
        [Route("SimpanListAsync")]
        [HttpPost("{listEntity}")]
        public async Task<IActionResult> SimpanListAsync([FromBody]List<DtOrgExperience> listEntity)
        {
        ...
        [HttpPost("{listEntity}")]
        public async Task<IActionResult> UpdateListAsync([FromBody]List<DtOrgExperience> listEntity)
        {
        ...
        {
            var hasil = await general.ListDataAsync<DtOrgExperience, VW_DtOrgExperience>(searchParam.SearchFieldList, searchParam.SortColumn, searchParam.IsAscending, searchParam.TopTake);
            return Ok(hasil);
        }
        ...
        {
            var hasil = await general.SearchByIdAsync<DtOrgExperience>(id);
            return Ok(hasil);
        }
    }
}